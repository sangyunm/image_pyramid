...
void box_filter_8u_c1(Mat CPUinput, unsigned short* CPUoutput, const int widthStep, const int filterWidth, const int filterHeight)
{
    ...
    */
    ...
    Mat temp_mat = Mat(HEIGHT, WIDTH, CV_16U, CPUoutput);
    Mat temp_mat_p(temp_mat, Rect(0, 0, 10, 10));
    cout << "ROLP_host" << temp_mat_p << endl;
...
//M:/Software/image_processing/Datasets/old data/outing/raw_NU_corrected/sixth
    ...
    fp.open("C:/Users/sxm41/Documents/image/sixth", ios::binary | ios::in);
...
////////// 24 bit //////////
    ...
    double minVal_input[1];
    double maxVal_input;
    minMaxLoc(input_whole, minVal_input, &maxVal_input);
    cout << "input : min = " << minVal_input[0] << ", max = " << maxVal_input << endl;
    input_whole = (input_whole - minVal_input[0]) / (maxVal_input - minVal_input[0]) * 255;
